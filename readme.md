...
* 多种事务形态可混合使用
* 高性能,大多数业务系统瓶颈在数据库，若不启用框架的幂等功能，对业务数据库的额外消耗仅为写入25字节的一行
...
* SAGA
	* 其余与传统补偿相似
	* 但区别为，正向操作和反向操作都不在主控事务执行过程中执行，因此可以减少事务记录加锁时间
	* 若追求高性能，可以用SAGA替代传统补偿，但其主控方代码形态会有稍有变化。
	* 本框架实现的SAGA并非传统的SAGA,可以将其类比为异步TCC，相比于传统SAGA的区别可以类比 传统补偿及TCC的关系
...
目前MASTER版本为1.0.0-SNAPSHOT，升级了一个大版本，计划引入较多功能，目前该大版本主要功能已开发完毕，后续只做BUGFIX，或引入轻量功能增强，不会改动主要代码逻辑及会保证1.0版本内的兼容性，其相对于之前的版本主要区别为：
...
### 传统补偿
传统补偿模式不支持往下嵌套其他事务，但是其他事务形态内可以嵌套传统补偿事务
...
* 其使用形态与其他的形态有所区别，DEMO短期内会补上，或请先参考UT案例
